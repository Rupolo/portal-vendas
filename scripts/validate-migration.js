...
function validateMigration() {
  ...
  // Check 10: Generated client.ts or models.ts
  if (check9.passed) {
    const clientPath = path.join(prismaGeneratedPath, "client.ts");
    const modelsPath = path.join(prismaGeneratedPath, "models.ts");
    const hasClientOrModels = fs.existsSync(clientPath) || fs.existsSync(modelsPath);
    
    const check10 = {
      name: "Prisma type definitions generated (client.ts or models.ts)",
      passed: hasClientOrModels,
    };
    if (!check10.passed) {
      check10.error = "Neither client.ts nor models.ts found";
    }
    results.checks.push(check10);

    if (check10.passed) {
      const sourceFile = fs.existsSync(clientPath) ? clientPath : modelsPath;
      const typesContent = fs.readFileSync(sourceFile, "utf8");
